...
CONFIG = {
    ...
    "start_range": 0x349b84b64310000000,
...
class JumpCounter:
    def __init__(self, manager):
        ...
        self.jump_stats = manager.dict({
            ...
            'low_entropy': manager.Value('i', 0),
            'window_pattern': manager.Value('i', 0)
        })
...
def calculate_entropy_score(zone):
    """Вычисляет оценку энтропии для зоны (0-1, где 1 - максимальная энтропия)"""
    if len(zone) == 0:
        return 0
    ...
    for count in char_counts.values():
        p = float(count) / len(zone)
        entropy -= p * np.log2(p)
    
    max_entropy = np.log2(len(set(zone))) if len(set(zone)) > 1 else 1
    return entropy / max_entropy if max_entropy > 0 else 0

def detect_sequences(zone):
    if len(zone) < CONFIG['min_sequence_length']:
        return 0
        
    seq_types = []
    for i in range(len(zone)-CONFIG['min_sequence_length']+1):
        window = zone[i:i+CONFIG['min_sequence_length']]
        if window.isdigit():
            if all(ord(window[j+1]) == ord(window[j])+1 for j in range(len(window)-1)):
                seq_types.append('digit_inc')
            elif all(ord(window[j+1]) == ord(window[j])-1 for j in range(len(window)-1)):
                seq_types.append('digit_dec')
        elif window.isalpha():
            if all(ord(window[j+1].lower()) == ord(window[j].lower())+1 for j in range(len(window)-1)):
                seq_types.append('alpha_inc')
            elif all(ord(window[j+1].lower()) == ord(window[j].lower())-1 for j in range(len(window)-1)):
                seq_types.append('alpha_dec')
    
    return len(seq_types)

def analyze_with_sliding_window(zone):
    """Анализ с использованием скользящего окна"""
    window_size = CONFIG['window_size']
    max_jump = 0
    
    for i in range(len(zone) - window_size + 1):
        window = zone[i:i+window_size]
        jump = analyze_window_pattern(window)
        max_jump = max(max_jump, jump)
    
    return max_jump

def analyze_window_pattern(window):
    """Анализ паттернов в окне"""
    # 1. Повторы
    if detect_repeats_numba(window) >= CONFIG['max_repeats']:
        return 0x1000
    ...
    if len(window) >= 3:
        if window.isdigit():
            if ord(window[1]) == ord(window[0])+1 and ord(window[2]) == ord(window[0])+2:
                seq_count += 1
            elif ord(window[1]) == ord(window[0])-1 and ord(window[2]) == ord(window[0])-2:
                seq_count += 1
        elif window.isalpha():
            if ord(window[1].lower()) == ord(window[0].lower())+1 and ord(window[2].lower()) == ord(window[0].lower())+2:
                seq_count += 1
            elif ord(window[1].lower()) == ord(window[0].lower())-1 and ord(window[2].lower()) == ord(window[0].lower())-2:
                seq_count += 1
    
    if seq_count > 0:
        return 0x100 * seq_count
    
    # 3. Все цифры/буквы
    if all(c.isdigit() for c in window):
        return 0x100
...
def analyze_17_chars(key_hex):
    ...
    # Анализ энтропии
    entropy_score = calculate_entropy_score(last_17)
    
    # Скользящее окно
    window_analysis = analyze_with_sliding_window(last_17)
...
def calculate_jump(key_hex, thread_id, jump_counter):
    ...
    # 1. Проверка энтропии (высокоприоритетный прыжок)
    if checks['entropy_score'] < CONFIG['min_entropy_threshold']:
        jump_size = 0x1000000000  # Большой прыжок для очень низкой энтропии
        jump_type = 'low_entropy'
        speed_gain = jump_size / 1000
        jump_counter.increment(jump_type, jump_size, speed_gain)
        new_pos = original + jump_size
        print(f"{Fore.RED}[Поток {thread_id}] LOW ENTROPY прыжок {jump_size:,} (энтропия: {checks['entropy_score']:.2f}){Style.RESET_ALL}")
        return min(new_pos, CONFIG['end_range'])
    
    # 2. Результаты скользящего окна
    window_jump = checks['window_analysis']
    if window_jump > 0:
        jump_type = 'window_pattern'
        speed_gain = window_jump / 100
        jump_counter.increment(jump_type, window_jump, speed_gain)
        new_pos = original + window_jump
        print(f"{Fore.BLUE}[Поток {thread_id}] WINDOW прыжок {window_jump:,}{Style.RESET_ALL}")
        return min(new_pos, CONFIG['end_range'])
    
    # 3. Высокоприоритетные паттерны
...
def print_status(stats, last_keys, jump_counter):
    ...
    print(f"  Низкая энтропия: {jump_counter.jump_stats['low_entropy'].value:,}")
    print(f"  Паттерны в окне: {jump_counter.jump_stats['window_pattern'].value:,}")
