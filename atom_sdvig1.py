...
class JumpCounter:
    def __init__(self, manager):
        ...
        self.jump_stats = manager.dict({
            'atomic_high': manager.Value('i', 0),
            'seq_high': manager.Value('i', 0),
            'adaptive_mid': manager.Value('i', 0),
            'low_repeat': manager.Value('i', 0),
...
def check_sequence(s):
    ...
    desc = all(ord(s[i+1]) == ord(s[i])-1 for i in range(len(s)-1))
    return asc or desc

def analyze_17_chars(key_hex):
    last_17 = key_hex[-17:] if len(key_hex) >= 17 else key_hex.zfill(17)[-17:]
    
    # Разбиваем на разряды: 5 разрядов (3+3+3+3+5)
    digits = [
        last_17[:3],    # Самый высокий разряд
        last_17[3:6],   # Высокий разряд
        last_17[6:9],   # Средний разряд
        last_17[9:12],  # Низкий разряд
        last_17[12:]    # Самый низкий разряд (5 символов)
    ]
    
    # Анализ каждого разряда
    digit_analysis = []
    for i, d in enumerate(digits):
        analysis = {
            'position': i,
            'value': d,
            'is_repeating': len(set(d)) == 1,
            'repeat_char': d[0] if len(set(d)) == 1 else None,
            'is_sequence': check_sequence(d),
            'is_digits': d.isdigit(),
            'is_alpha': d.isalpha(),
            'is_mixed': not (d.isdigit() or d.isalpha())
        }
        digit_analysis.append(analysis)
...
def calculate_jump(key_hex, thread_id, jump_counter):
    ...
    digit_analysis, checks = analyze_17_chars(key_hex)
    
    # 1. Проверка самых высоких разрядов (первые 3 символа)
    for i in range(2):  # Анализируем два самых высоких разряда
        analysis = digit_analysis[i]
        if analysis['is_repeating']:
            # Для повторяющихся символов в высоких разрядах используем атомарное изменение
            jump_size = 16 ** (15 - i*3)  # Большой прыжок, но не пропускаем валидные зоны
            jump_type = 'atomic_high'
            jump_counter.increment(jump_type, jump_size, 1)
            new_pos = original + 1  # Атомарное изменение вместо прыжка
            print(f"{Fore.CYAN}[Поток {thread_id}] Атомарное изменение в высоком разряде {i}{Style.RESET_ALL}")
            return new_pos
            
        if analysis['is_sequence']:
            jump_size = 16 ** (14 - i*3)  # Средний прыжок для последовательностей
            jump_type = 'seq_high'
            jump_counter.increment(jump_type, jump_size, 1)
            new_pos = original + jump_size
            print(f"{Fore.BLUE}[Поток {thread_id}] Прыжок {jump_size:,} для последовательности в разряде {i}{Style.RESET_ALL}")
            return new_pos
    
    # 2. Проверка средних разрядов (разряды 2 и 3)
    for i in range(2, 4):
        analysis = digit_analysis[i]
        if analysis['is_repeating'] and analysis['repeat_char'] in '0123456789abcdef':
            # Для цифр/букв в средних разрядах используем адаптивные прыжки
            char_value = int(analysis['repeat_char'], 16)
            jump_size = (16 - char_value) * (16 ** (12 - i*3))
            jump_type = 'adaptive_mid'
            jump_counter.increment(jump_type, jump_size, 1)
            new_pos = original + jump_size
            print(f"{Fore.GREEN}[Поток {thread_id}] Адаптивный прыжок {jump_size:,} в разряде {i}{Style.RESET_ALL}")
            return new_pos
    
    # 3. Проверка самого низкого разряда (последние 5 символов)
    low_analysis = digit_analysis[4]
    if low_analysis['is_repeating']:
        repeat_len = len(low_analysis['value'])
        jump_size = 16 ** repeat_len
        jump_type = 'low_repeat'
        jump_counter.increment(jump_type, jump_size, 1)
        new_pos = original + jump_size
        print(f"{Fore.YELLOW}[Поток {thread_id}] Прыжок {jump_size:,} для повтора в низком разряде{Style.RESET_ALL}")
        return new_pos
    ...
    # 5. Последовательности в любом месте
    ...
    # 6. Минимальное увеличение, если ничего не найдено
    return original + 1
...
def print_status(stats, last_keys, jump_counter):
    ...
    print(f"  Атомарные (высокий): {jump_counter.jump_stats['atomic_high'].value:,}")
    print(f"  Последовательности (высокий): {jump_counter.jump_stats['seq_high'].value:,}")
    print(f"  Адаптивные (средний): {jump_counter.jump_stats['adaptive_mid'].value:,}")
    print(f"  Повторы (низкий): {jump_counter.jump_stats['low_repeat'].value:,}")
